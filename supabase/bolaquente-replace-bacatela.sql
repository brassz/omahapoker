-- Substitui Bacatela por Bola Quente + esconde Trinca no RTP ativo.
-- Rode no SQL Editor do Supabase.

-- Copia RTP da bacatela para bolaquente (se existir)
insert into public.game_rtp (game_id, enabled, rtp_percent, bet_counter, player_win_counter)
select 'bolaquente', enabled, rtp_percent, bet_counter, player_win_counter
from public.game_rtp
where game_id = 'bacatela'
  and not exists (select 1 from public.game_rtp where game_id = 'bolaquente');

insert into public.game_rtp (game_id, enabled, rtp_percent)
select 'bolaquente', false, coalesce((select s.rtp_percent from public.game_settings s where s.id = 1), 20)
where not exists (select 1 from public.game_rtp where game_id = 'bolaquente');
...
set maintenance_games = array_replace(maintenance_games, 'bacatela', 'bolaquente')
where maintenance_games @> array['bacatela']::text[];

delete from public.game_rtp where game_id in ('bacatela', 'bagatela');
