-- Adiciona Baccarat ao RTP e à manutenção.
-- Rode no SQL Editor do Supabase.

insert into public.game_rtp (game_id, enabled, rtp_percent)
select 'baccarat', false, coalesce((select s.rtp_percent from public.game_settings s where s.id = 1), 20)
where not exists (select 1 from public.game_rtp where game_id = 'baccarat');
